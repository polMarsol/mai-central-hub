...
- **UI / cuerpo / datos**: stack de sistema
  ...
  — todo lo demás: navegación, celdas de calendario, labels, botones,
...
- Objetivos táctiles (botones, selects de grupo, pestañas de día) de
  al menos 44×44px.
...
Dos usos permitidos, y solo esos dos — nada de hover genérico en cada
tarjeta ni animaciones de entrada repetidas sección a sección:

1. **Hover sutil en todo elemento interactivo** (enlaces de
   navegación, footer, selector de idioma, botones): un cambio leve
   — subrayado que aparece con `transform: scaleX()` desde
...
Fuera de esos dos casos, sin animaciones de entrada por scroll.
