...
export function RealCalendar({
  ...
  const initialMonth = useMemo(() => {
    const allDates = [...events.map((e) => new Date(e.date)), ...holidays.map((h) => new Date(`${h.date}T00:00:00Z`))];
    ...
    <div className="flex flex-col gap-10">
      <section className="flex flex-col gap-3" style={{ borderTop: `1px solid ${COLORS.hairline}`, paddingTop: "1.5rem" }}>
        <h2 className="font-display text-sm font-medium" style={{ color: COLORS.textPrimary }}>
          {t("startOfTermHeading")}
        </h2>
        <p className="text-sm" style={{ color: COLORS.textSecondary }}>
          {t("startOfTermSubtitle")}
        </p>
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
          {universities.map((uni) => (
            <div key={uni.code} className="flex flex-col gap-1">
              <span className="text-sm font-medium" style={{ color: COLORS.textPrimary }}>
                {uni.code}
              </span>
              <span className="text-sm" style={{ color: COLORS.textSecondary, fontVariantNumeric: "tabular-nums" }}>
                {startDateFormatter.format(new Date(`${uni.startDate}T00:00:00Z`))}
              </span>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  backgroundColor: holiday ? COLORS.backgroundSecondary : COLORS.background,
              ...
              >
                ...
                {holiday && (
                  <span className="truncate text-[10px] sm:text-xs" style={{ color: COLORS.textSecondary }}>
